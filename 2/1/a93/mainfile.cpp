...
float intm[20][200][3][2]; int k; float alpha, beta;
float gama;
...
{	int local;
	...
	//cout<<st<<" "<<dt<<" ";cout<<destinp[4];
		...
		/*for(i=0;i<globnii;i++)
		{
			...
			for(local=2;local<=globnii;local++)
			{
				s.edge_comp(local);
				/*cout<<"\npos="<<s.retpos();
...
*/
			...
			int c,d,e;//k=0;
				...
				{
					alpha=(i+1)*1.0/(NumOutputImages+1)*1.0;
					for(j=0;j<l;j++)
						{
							c=s.rettri(j,0);
							d=s.rettri(j,1);
							e=s.rettri(j,2);
							intm[i][j][0][0]=((1-alpha)*s.retptorigx(c-1) + alpha*deste[c-1][0]);
							intm[i][j][1][0]=((1-alpha)*s.retptorigx(d-1) + alpha*deste[d-1][0]);
							intm[i][j][2][0]=((1-alpha)*s.retptorigx(e-1) + alpha*deste[e-1][0]);

							intm[i][j][0][1]=((1-alpha)*s.retptorigy(c-1) + alpha*deste[c-1][1]);
							intm[i][j][1][1]=((1-alpha)*s.retptorigy(d-1) + alpha*deste[d-1][1]);
							intm[i][j][2][1]=((1-alpha)*s.retptorigy(e-1) + alpha*deste[e-1][1]);
							//k+=3;
						}
				}

				/*for(j=0;j<l;j++)
				{	cout<<"trianglr"<<j+1<<endl;
					for(int dapa=0;dapa<3;dapa++)
					{cout<<endl;cout<<intm[0][j][dapa][0]<<" "<<intm[0][j][dapa][1];}
					cout<<endl;
				}*/
				srcim.readImage(src);
				dstim.readImage(dest);
				float lrg_x,sml_x,lrg_y,sml_y;float tolo1,tolo2,tolo3;int sweg=0;int brhm,shiv;
					for(i=0;i<NumOutputImages;i++)
					{	alpha=(i+1)/(NumOutputImages+1);
						strcpy(intim[i].type ,srcim.type);
						intim[i].width=srcim.width;
						intim[i].height=srcim.height;
						intim[i].max_val=srcim.max_val;
						for(j=0;j<l;j++)
							{
								...
								for(brhm=sml_x;brhm<=lrg_x;brhm++)
								{
									for(shiv=sml_y;shiv<=lrg_y;shiv++)
									{
										...
										if(u.x*v.y!=u.y*v.x)//check for other condition also not done till now
										{
											...
											intim[i].color[(int)(brhm-1)][(int)(shiv-1)]=(int)((1-alpha)*srcim.color[(int)(P1.x-1)][(int)(P1.y-1)] + alpha*dstim.color[(int)(P2.x-1)][(int)(P2.y-1)]);
